...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- one continuous line sequence across the whole file (main poem + Threnos) -->
    ...
    <xsl:template match="tei:body/@xml:base">
        <xsl:attribute name="xml:base">urn:cts:engLit:shakespeare.pht.globe</xsl:attribute>
    </xsl:template>

    <!-- line numbering is now continuous, not per-stanza, so the line unit
         must address by @n rather than position() (which would reset per
         stanza context) -->
